"""off-policy(DrQ-v2)离线评估入口 —— arm A 族统一脚本(spr_z / pixels / spr_coadapt)。

替代 val/drq_student.py。--cfg 指定 val cfg 模块(含 OUT_DIR / VAL 参数):

    ./IsaacLab/isaaclab.sh -p val/offpolicy.py --cfg drq_student_cfg --obs-mode spr_z

评估动作 = 确定性 μ。final_de/xe 在 env.step 前快照目标,避开 _reset_idx 重采样伪影。
"""
...
parser = argparse.ArgumentParser(description="Off-policy DrQ-v2 offline val (arm A family).")
parser.add_argument("--cfg", type=str, required=True, help="val cfg 模块名,如 drq_student_cfg")
...
from src.rl.agent import DrQV2Agent
    ...
    with torch.no_grad():
        for episode_id in range(int(num_episodes)):
            ...
            for t in range(steps):
                ...
                step_end_d = env.end_d.clone()  # env.step 会重采样目标,先快照
...
def main():
    ...
    if not policy_paths:
        raise FileNotFoundError(f"no drq_* checkpoints: {update_dir}")
    ...
    print(f"[INFO] offline val cfg={args_cli.cfg} mode={args_cli.obs_mode} checkpoints={len(policy_paths)} "
    ...
    try:
        with out_csv.open("w", newline="", encoding="utf-8") as file, \
            ...
            for path in policy_paths:
                agent, _ckpt = load_agent(path, device)
                step = step_num(path)
                traj_buf = TrajBuffer()
                metrics = val(env, agent, spr, num_episodes, step, traj_buf, env0_writer)
                row = {
                    "checkpoint": path.name, "kind": "drq", "spr_checkpoint": spr_ckpt_name,
