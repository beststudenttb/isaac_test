"""Offline validate fixed SPR-encoder student checkpoints.

Run from the project root:

    ./IsaacLab/isaaclab.sh -p val/spr_fixed_student.py
"""
...
parser = argparse.ArgumentParser(description="Offline validate fixed SPR-encoder student checkpoints.")
...
VAL_FIELDS = [
    "checkpoint", "update", "step", "num_envs", "num_episodes", "episodes",
...
TRAJ_FIELDS = [
    ...
    "a_x", "a_y", "a_w", "reward", "done", "success", "fail", "timeout",
...
def build_model(ckpt: dict, device: torch.device) -> SPRFixedActorCritic:
    model_cfg = ckpt["model_cfg"]
    encoder = SPRStateNet(**model_cfg["encoder_cfg"]).to(device)
...
def traj_row(env: MDPStudentEnv, step: int, update: int, t: int, env_id: int, reward: torch.Tensor, done: bool) -> dict:
    ...
    return {
        "phase": "offline_val", "step": int(step), "update": int(update), "t": int(t), "env_id": int(env_id),
        "px_x": f"{float(env.last_px_x[env_id].item()):.4f}", "dist": f"{float(env.last_dist[env_id].item()):.4f}",
        "end_d": f"{float(env.end_d[env_id].item()):.4f}", "end_x": f"{float(env.end_x[env_id].item()):.4f}",
        "a_x": f"{float(action[0]):.4f}", "a_y": f"{float(action[1]):.4f}", "a_w": f"{float(action[2]):.4f}",
        "reward": f"{float(reward.item()):.4f}", "done": int(done),
        "success": int(bool(env.last_success[env_id].item())), "fail": int(bool(env.last_fail[env_id].item())),
...
def val(env: MDPStudentEnv, model: SPRFixedActorCritic, num_episodes: int, update: int, step: int, traj_writer, env0_writer=None) -> dict[str, float]:
    steps = int(cfg.VAL_STEPS) if int(cfg.VAL_STEPS) > 0 else int(env.max_episode_length)
    episodes = env.num_envs * int(num_episodes)
    success_count = fail_count = timeout_count = 0
            ...
            for t in range(steps):
                ...
                if traj_writer is not None:
                    for env_id in torch.nonzero(active, as_tuple=False).flatten().tolist():
                        traj_writer.writerow(traj_row(env, step, update, row_t, int(env_id), reward[int(env_id)], bool(done[int(env_id)])))
                if env0_writer is not None and bool(active[0].item()):
                    env0_writer.writerow(traj_row(env, step, update, row_t, 0, reward[0], bool(done[0])))
                if torch.any(done):
...
def best_key(row: dict) -> tuple:
    return float(row["success_rate"]), -float(row["fail_rate"]), -float(row["timeout_rate"]), float(row["mean_return"])


def write_config(path: Path, env: MDPStudentEnv):
    lines = [
        "run_name = 'spr_fixed_student'",
...
def main():
    ...
    paths = sorted(update_dir.glob("ppo_*.pt"), key=update_num)
    start = int(args_cli.start)
    stride = int(args_cli.stride)
    paths = [p for p in paths if update_num(p) >= start and (update_num(p) - start) % stride == 0]
    if not paths:
        raise FileNotFoundError(f"no checkpoints found: {update_dir}")
    ...
    best_row = None
    best_path = None
    print(f"[INFO] offline val run=spr_fixed_student checkpoints={len(paths)} envs={env.num_envs} episodes={args_cli.num_episodes} device={env.device}")
    try:
        with (root / "offline_val.csv").open("w", newline="", encoding="utf-8") as file, (root / "traj_offline_env0.csv").open("w", newline="", encoding="utf-8") as env0_file:
            ...
            for path in paths:
                ...
                update = int(ckpt.get("update", update_num(path)))
                step = int(ckpt.get("step", -1))
                traj_buf = TrajBuffer()
                info = val(env, model, int(args_cli.num_episodes), update, step, traj_buf, env0_writer)
                row = {
                ...
                print_log([
                    ...
                    ("val", [("episodes", row["episodes"]), ("success", row["success_rate"]), ("fail", row["fail_rate"]), ("timeout", row["timeout_rate"])]),
                ])
        print(f"[INFO] best checkpoint={best_path.name} success={best_row['success_rate']:.3f} fail={best_row['fail_rate']:.3f} timeout={best_row['timeout_rate']:.3f}")
    finally:
        env.close()
