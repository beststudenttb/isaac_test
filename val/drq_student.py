"""Offline validate DrQ student checkpoints.

drq_*.pt 自带 agent_cfg;spr_z 模式配同目录 spr_encoder.pt 的冻结 encoder。
评估动作 = 确定性 μ(无探索噪声)。

Run from the project root:

    ./IsaacLab/isaaclab.sh -p val/drq_student.py --obs-mode spr_z
"""
...
parser = argparse.ArgumentParser(description="Offline validate DrQ student checkpoints.")
...
from src.drqv2 import DrQV2Agent
...
def root_dir() -> Path:
    base = Path(cfg.OUT_DIR) if args_cli.obs_mode == "spr_z" else Path(cfg.PIXELS_OUT_DIR)
...
def load_spr(root: Path, device: torch.device) -> SPRStateNet:
    path = root / str(cfg.SPR_CKPT)
    if not path.exists():
...
def load_agent(path: Path, device: torch.device) -> tuple[DrQV2Agent, dict]:
    ckpt = torch.load(path, map_location=device)
    if str(ckpt.get("obs_mode")) != str(args_cli.obs_mode):
        raise ValueError(f"checkpoint obs_mode={ckpt.get('obs_mode')} != cli obs_mode={args_cli.obs_mode}: {path}")
    agent = DrQV2Agent(**ckpt["agent_cfg"], device=device)
...
def write_config(path: Path, env: MDPStudentEnv):
    lines = [
        "run_name = 'drq_student'",
        f"obs_mode = {str(args_cli.obs_mode)!r}",
...
def main():
    ...
    policy_paths = sorted(update_dir.glob("drq_*.pt"), key=step_num)
    if not policy_paths:
        raise FileNotFoundError(f"no drq_* checkpoints found: {update_dir}")
    start = int(args_cli.start)
    stride = int(args_cli.stride)
    if stride <= 0:
        raise ValueError(f"stride must be positive: {stride}")
    policy_paths = [path for path in policy_paths if step_num(path) >= start]
    policy_paths = policy_paths[::stride]
    if not policy_paths:
        raise FileNotFoundError(f"no checkpoints found after start={start}, stride={stride}: {update_dir}")

    env = make_env()
    device = torch.device(env.device)
    spr = load_spr(root, device) if args_cli.obs_mode == "spr_z" else None
    ...
    print(
        f"[INFO] offline val run=drq_student mode={args_cli.obs_mode} checkpoints={len(policy_paths)} "
    ...
    try:
        with out_csv.open("w", newline="", encoding="utf-8") as file, env0_traj_csv.open("w", newline="", encoding="utf-8") as env0_file:
            ...
            for policy_path in policy_paths:
                agent, ckpt = load_agent(policy_path, device)
                step = int(ckpt.get("step", step_num(policy_path)))
                update = step // max(int(env.num_envs), 1)
                traj_buf = TrajBuffer()
                info = val(
                    env,
                    agent,
                ...
                row = {
                    ...
                    "kind": "drq",
                    "spr_checkpoint": str(cfg.SPR_CKPT) if args_cli.obs_mode == "spr_z" else policy_path.name,
                    "update": update,
                    "step": step,
        ...
        print(
            f"[INFO] best checkpoint={best_path.name} "
