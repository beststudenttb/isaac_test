"""Offline validate JSRL SPR-PPO checkpoints。

全部是 full_*.pt(自包含,含冻结 encoder),不需要独立的 SPR 基文件。

    ./IsaacLab/isaaclab.sh -p val/spr_jsrl.py --noise
"""
...
parser = argparse.ArgumentParser(description="Offline validate JSRL SPR-PPO checkpoints.")
...
def build_model(ckpt: dict, device: torch.device) -> SPRActorCritic:
    spr = SPRStateNet(**ckpt["spr_cfg"]).to(device)
    model = SPRActorCritic(
        spr=spr, goal_dim=2, act_dim=3,
...
def traj_row(env, phase, step, update, t, env_id, reward, done) -> dict:
...
def val(env, model, num_episodes, update, step, traj_writer, env0_traj_writer=None) -> dict[str, float]:
...
def main():
    ...
    policy_paths = sorted(update_dir.glob("full_*.pt"), key=update_num)
    if not policy_paths:
        raise FileNotFoundError(f"no full_*.pt checkpoints found: {update_dir}")
    start = int(args_cli.start)
    stride = int(args_cli.stride)
    if stride <= 0:
        raise ValueError(f"stride must be positive: {stride}")
    policy_paths = [path for path in policy_paths if update_num(path) >= start][::stride]
    if not policy_paths:
        raise FileNotFoundError(f"no checkpoints after start={start}, stride={stride}: {update_dir}")
    ...
    print(f"[INFO] offline val run=spr_jsrl checkpoints={len(policy_paths)} "
        ...
        with out_csv.open("w", newline="", encoding="utf-8") as file, env0_traj_csv.open("w", newline="", encoding="utf-8") as env0_file:
            ...
            for policy_path in policy_paths:
                ...
                row = {
                    ...
                    "update": update,
                    "step": step,
                    "jsrl_h": float(ckpt.get("jsrl_h", -1)),
                ...
                print(f"[val] {policy_path.name} update={update} h={row['jsrl_h']:.0f} "
                      f"success={row['success_rate']:.3f} fail={row['fail_rate']:.3f} timeout={row['timeout_rate']:.3f} "
                      f"return={row['mean_return']:.2f} de={row['final_de_mean']:.3f}")

        if best_row is None or best_path is None:
            raise RuntimeError("offline val did not produce best checkpoint")
        print(f"[INFO] best checkpoint={best_path.name} update={best_row['update']} "
