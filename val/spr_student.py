"""Offline validate SPR student checkpoints.

ppo_*.pt pairs with the frozen SPR in stage1_end.pt; full_*.pt is self-contained.
...
parser = argparse.ArgumentParser(description="Offline validate SPR student checkpoints.")
...
def load_checkpoint(model: SPRActorCritic, base_state: dict, path: Path, device: torch.device) -> dict:
    ckpt = torch.load(path, map_location=device)
    if ckpt_kind(path) == "ppo":
...
def main():
    ...
    spr_path = root / str(cfg.SPR_CKPT)
    if not spr_path.exists():
        raise FileNotFoundError(f"SPR checkpoint not found: {spr_path}")

    policy_paths = sorted(
        list(update_dir.glob("ppo_*.pt")) + list(update_dir.glob("full_*.pt")),
        key=update_num,
    )
    if not policy_paths:
        raise FileNotFoundError(f"no ppo_*/full_* checkpoints found: {update_dir}")
    start = int(args_cli.start)
    stride = int(args_cli.stride)
    if stride <= 0:
        raise ValueError(f"stride must be positive: {stride}")
    policy_paths = [path for path in policy_paths if update_num(path) >= start]
    ...
    print(
        f"[INFO] offline val run=spr_student checkpoints={len(policy_paths)} spr={spr_path.name} "
    ...
    try:
        with out_csv.open("w", newline="", encoding="utf-8") as file, env0_traj_csv.open("w", newline="", encoding="utf-8") as env0_file:
            ...
            for policy_path in policy_paths:
                kind = ckpt_kind(policy_path)
                ckpt = load_checkpoint(model, base_state, policy_path, device)
                ...
                row = {
                    ...
                    "kind": kind,
                    "spr_checkpoint": spr_path.name if kind == "ppo" else policy_path.name,
                    "update": update,
                    "step": step,
        ...
        print(
            f"[INFO] best checkpoint={best_path.name} kind={best_row['kind']} "
