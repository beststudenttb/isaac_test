#!/usr/bin/env bash
# 分阶段 SPR-PPO Combo A(forward -> jsrl)训练 + 离线 val 一条龙。
# 用法(默认 GPU0、frozen、noise env):
#     ./scripts/run_spr_phased_a.sh
# 覆盖:GPU=1 PHASED_ENCODER=cotrain ./scripts/run_spr_phased_a.sh
set -e
