...
ap.add_argument("--out", default="figs/direct_contrib.png")
a=ap.parse_args()
...
def contrib(e,img):
    ...
    out=0.0
    for s,p in enumerate(ps):                         # p: (1,C,H,W)
...
for j,(arm,rel) in enumerate(ARMS):
    ...
    for i,(c,r) in enumerate(sel):
        ...
        h=contrib(e,img); h=h/(h.max()+1e-9)
        ax[i][j+1].imshow(rgb); ax[i][j+1].imshow(h,cmap="jet",alpha=0.55,vmin=0,vmax=1)
        ax[i][j+1].set_xticks([]); ax[i][j+1].set_yticks([])
        if i==0: ax[i][j+1].set_title(arm,fontsize=11)
        print(f"{c:12s}{arm:5s} ok",flush=True)
plt.suptitle("每个位置把 z 推了多远 ‖W·p(y,x)‖(直接用表征网络权重算,无拟合/无扰动/无梯度)",fontsize=12)
