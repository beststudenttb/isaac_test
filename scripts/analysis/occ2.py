...
ap.add_argument("--out", default="figs/occlusion_v2.png")
a=ap.parse_args()
...
@torch.no_grad()
def imap(e,img):
    med=img.float().flatten(1).median(1).values.view(3,1,1)   # 该图中位色 = 地板/墙灰
    ...
    for i in range(0,len(POS),64):
        ...
        for k,(y,x) in enumerate(ch): b[k,:,y:y+a.patch,x:x+a.patch]=med
...
plt.suptitle("遮挡该处后表征的变化 ‖Δz‖/‖z‖(已减去背景基线;填该图中位色)",fontsize=12)
