...
plt.xlabel("场上多了这个干扰物(目标红球位置不变,几何逐像素相同)")
plt.title("换个干扰物,表征被推多远(无拟合/无扰动,全部在分布内)",fontsize=13)
plt.legend(); plt.grid(axis="y",alpha=0.3); plt.tight_layout()
plt.savefig("figs/distractor_shift.png",dpi=150); print("-> figs/distractor_shift.png",flush=True)
