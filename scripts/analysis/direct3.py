...
ap.add_argument("--out", default="figs/direct_contrib_v3.png")
a=ap.parse_args()
...
_rnd.seed(0)
ref_rows=_rnd.sample(range(len(rows)),REF)
def refstat(e):
    acc=[]
    for j in ref_rows:
        acc.append(contrib(e,read_image(str(D/rows[j]["img"]))))
    A_=np.stack(acc); return A_.mean(0), A_.std(0)+1e-6
...
for j,(arm,rel) in enumerate(ARMS):
    ...
    mu_r,sd_r=refstat(e); print(f"{arm} 参考统计完成",flush=True)
    for i,(c,r) in enumerate(sel):
        ...
        h=(contrib(e,img)-mu_r)/sd_r        # 逐像素 z-score:减掉静态权重结构,只留这一帧的内容
        h=np.clip(h/max(np.percentile(h,99.5),1e-9),0,1)
...
plt.suptitle("每个位置把 z 推了多远(只用 FPN p2 56×56,逐像素对 200 帧参考分布做 z-score,去掉静态权重结构)",fontsize=12)
