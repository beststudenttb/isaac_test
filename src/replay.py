...
class ReplayBuffer:
    ...
    def __init__(
        ...
        self.images = torch.zeros((T, N, *image_shape), dtype=torch.uint8)
        ...
        self.term_images = torch.zeros((pool, *image_shape), dtype=torch.uint8)
    ...
    def sample(self, batch_size: int, device: torch.device, with_images: bool = True) -> dict[str, torch.Tensor]:
        ...
        n = self.n_step
        if self.size <= n:
            raise ValueError(f"buffer too small to sample: size={self.size} n_step={n}")
        T = self.capacity
        max_a = self.size - 1 - n
