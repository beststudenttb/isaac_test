...
K_X = 0.02  # approach 阶段 px_x 误差改善 reward 系数。
K_D = 1.0  # approach 阶段 dist 误差改善 reward 系数。
K_STOP_A = 0.5  # stop 区域内动作平方惩罚系数。

SIG_X = 10.0  # stop 区域二维高斯的图像 x 标准差，单位 px。
SIG_D = 0.2  # stop 区域二维高斯的距离标准差，单位 m。

R_FIND = 1.0  # 从看不到球到看到球时的一次性 reward。
R_LOST = -1.5  # 从看到球到看不到球时的一次性 penalty。
R_STOP_IN = 1.0  # 第一次进入 stop 区域时的一次性 reward。
R_STOP_OUT = -1.5  # 从 stop 区域离开时的一次性 penalty。
R_STOP_Q = 0.3  # stop 区域内二维高斯势场 reward 系数。
R_SUCCESS = 10.0  # success done 时的终止 reward。
