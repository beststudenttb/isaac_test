"""Standalone IsaacLab ball robot environment for DrQ-v2.

The environment returns RGB camera images as policy observations:

    obs["policy"].shape == (num_envs, 3, image_height, image_width)

Actions are three continuous values in [-1, 1]:
...
from drq_env import task_cfg


@configclass
class DrqBallEnvCfg(DirectRLEnvCfg):
...
class DrqBallEnv(DirectRLEnv):
    cfg: DrqBallEnvCfg

    def __init__(self, cfg: DrqBallEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _standardize_xforms(self, env_path: str):
        paths = (f"{env_path}/Robot", f"{env_path}/Robot/head", f"{env_path}/Robot/head/eye", f"{env_path}/target")
        for path in paths:
    ...
    def _get_observations(self) -> dict:
        return {"policy": self.camera_rgb_chw()}
...
def make_env(cfg: DrqBallEnvCfg | None = None) -> DrqBallEnv:
    return DrqBallEnv(cfg or DrqBallEnvCfg())
