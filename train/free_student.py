"""任务三:用 stage2 的冻结表征从零训 PPO(交接文档 2026-09-03 §3)。

    ./IsaacLab/isaaclab.sh -p train/free_student.py --arm A

encoder 冻结、teacher 全程为 0。唯一变量是 --arm 选的那个表征。
"""
...
parser = argparse.ArgumentParser(description="Task three: PPO from scratch on a frozen stage-2 representation.")
parser.add_argument("--arm", required=True)  # 任意臂名:{ENCODER_ROOT}/{arm}/encoder_final.pt;"init" 特殊 = encoder_init.pt  # init = stage2 之前的共享随机初值,对照用
...
parser.add_argument("--stage2-steps", type=int, default=None)     # 覆盖 cfg.STAGE2_STEPS:线性退火阶段的环境步数
parser.add_argument("--teacher-floor", type=float, default=0.0)  # 退火结束后 teacher 系数的下限(teacher 常驻锚)
parser.add_argument("--adapter-hidden", type=int, default=0)     # >0:两层适配器
parser.add_argument("--init-from", default=None)                  # 目录:含 adapter.pt / head.pt(scripts/bc_pretrain_adapter.py 的产物),初始化适配器与 actor
parser.add_argument("--adapter-lr", type=float, default=None)     # 适配器单独学习率(默认同 LR)
...
if args_cli.teacher_loss is not None:
    cfg.TEACHER_LOSS = float(args_cli.teacher_loss)
if args_cli.stage1_steps is not None:
    cfg.STAGE1_STEPS = int(args_cli.stage1_steps)
if args_cli.stage2_steps is not None:
    cfg.STAGE2_STEPS = int(args_cli.stage2_steps)
...
from src.score_env import ScoreNoiseStudentEnvCfg, ScoreStudentEnvCfg, make_score_noise_student_env, make_score_student_env
...
def out_dir() -> Path:
    ...
    path = Path(f"{cfg.OUT_ROOT}_{args_cli.arm}{suffix}{args_cli.tag}")
...
def encoder_path() -> Path:
    ...
    if args_cli.arm == "init":
        return root / "encoder_init.pt"
    return root / str(args_cli.arm) / "encoder_final.pt"
...
def load_teacher(device: torch.device):
    """只用于轨迹 CSV 的诊断列(teacher 在这一步会怎么做)。不进 loss:
    STAGE1_STEPS=STAGE2_STEPS=0 使 stage_of 恒为 3、teacher_coef 恒 0,
    TEACHER_LOSS=0.0 再锁一道。"""
...
def stage_teacher_coef(step: int, stage: int) -> float:
    ...
    if stage == 2:
        ...
        return max(float(cfg.TEACHER_LOSS) * max(1.0 - frac, 0.0), float(args_cli.teacher_floor))
    return float(args_cli.teacher_floor)


def std_info(model: FreeFeatureActorCritic) -> dict[str, float]:
    std = torch.exp(model.log_std.detach())
    return {"std_mean": float(std.mean().cpu()), "std_min": float(std.min().cpu()), "std_max": float(std.max().cpu())}


def encoder_cfg() -> dict:
    return dict(FREE_SPATIAL_CONFIG)
...
def model_cfg_dict() -> dict:
    return {
        ...
        "goal_dim": 2,
        "act_dim": 2 if cfg.NO_LATERAL else 3,
        ...
        "freeze_encoder": True,
        "adapter": bool(args_cli.adapter),
        "adapter_hidden": int(args_cli.adapter_hidden),
        "init_from": args_cli.init_from,
        "adapter_lr": args_cli.adapter_lr,
    }


def save_model(path: Path, model: FreeFeatureActorCritic, opt: torch.optim.Optimizer, update: int, step: int, best: float, teacher_coef: float, info: dict):
    torch.save(
        {
...
def traj_row(env: MDPStudentEnv, step: int, update: int, t: int, reward: torch.Tensor, done: bool, teacher_a: torch.Tensor) -> dict:
    ...
    if cfg.NO_LATERAL:  # teacher 也是 [a_x, a_w];CSV 仍记三列
        teacher_a = [float(teacher_a[0]), 0.0, float(teacher_a[1])]
...
def main() -> None:
    if args_cli.seed is not None:
        torch.manual_seed(int(args_cli.seed)); np.random.seed(int(args_cli.seed)); random.seed(int(args_cli.seed))
    seed = int(cfg.SEED)
    random.seed(seed)
    np.random.seed(seed)
    ...
    model = (AdapterActorCritic if args_cli.adapter else FreeFeatureActorCritic)(
        encoder=encoder,
        **({"adapter_hidden": int(args_cli.adapter_hidden)} if args_cli.adapter else {}),
        goal_dim=2,
    ...
    if args_cli.init_from:
        ini = Path(args_cli.init_from)
        model.adapter.load_state_dict(torch.load(ini / "adapter.pt", map_location=device))
        model.actor.load_state_dict(torch.load(ini / "head.pt", map_location=device))
        print(f"[INFO] adapter/actor 从 {ini} 初始化(VA 预训练)", flush=True)
    if args_cli.adapter and args_cli.adapter_lr is not None:
        ad_ids = {id(p) for p in model.adapter.parameters()}
        opt = torch.optim.Adam([
            {"params": [p for p in model.parameters() if p.requires_grad and id(p) not in ad_ids], "lr": float(cfg.LR)},
            {"params": [p for p in model.adapter.parameters()], "lr": float(args_cli.adapter_lr)},
        ], eps=1e-5)
    else:
    ...
    total_steps = int(cfg.NUM_ENVS * cfg.N_STEPS * args_cli.updates) if args_cli.updates else int(cfg.TOTAL_STEPS)
    ...
    print(
        f"[INFO] free student PPO arm={args_cli.arm} out={log_dir} envs={env.num_envs} "
        f"total_steps={total_steps} updates={updates} encoder={encoder_path()} "
        f"angle_deg={cfg.ANGLE_DEG} teacher_loss={cfg.TEACHER_LOSS} std_init={cfg.STD_INIT} device={env.device}"
    )
    ...
    try:
        for update in range(1, updates + 1):
            ...
            if args_cli.freeze_adapter_at_stage3 and stage == 3 and hasattr(model, "adapter") and any(p.requires_grad for p in model.adapter.parameters()):
                model.adapter.requires_grad_(False)
                for p in model.adapter.parameters():
                    p.grad = None
                print(f"[INFO] update {update}: teacher 退完,适配器冻结", flush=True)
            rollout = CNNRollout(
            ...
            # encoder 冻结,特征在这里一次性算好;PPO 的 minibatch 之后只碰 256 维向量,
            # 不再重复过 ResNet18(否则 BATCH_SIZE=1024 会直接爆显存)。
            batch = dataclasses.replace(batch, images=model.encode_rollout(batch.images))
            if teacher_coef > 0.0:
