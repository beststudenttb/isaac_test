"""Fixed SPR-encoder visual student PPO config."""

from pathlib import Path


OUT_DIR = Path("./models/rl/spr_fixed_student")  # 固定 SPR encoder student 输出目录。
RANDOM_STOP_OUT_DIR = Path("./models/rl/spr_fixed_student_random_stop")  # --random-stop 输出目录。
...
Z_DIM = 128  # encoder 输出特征维度。
FPN_DIM = 128  # FPN 通道数。
SPR_HIDDEN = 256  # SPR encoder 内部 hidden。
SPR_POOL = 7  # encoder 池化尺寸。
SPR_PRETRAINED = True  # 对齐周末 SPR 配置；若要纯随机权重改为 False。
FREEZE_STEM_LAYERS = True  # 对齐 SPR 配置。
FREEZE_ENCODER = True  # 固定 encoder，只训练 actor/critic/std。
