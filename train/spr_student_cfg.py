...
STAGE2_UPDATES = 350  # 阶段2 update 数:冻结 SPR,student PPO + teacher BC(全权重)。
STAGE3_UPDATES = 200  # 阶段3 update 数:冻结 SPR,teacher 权重线性退火到 0;0 表示跳过(硬切);其余归阶段4。
...
TEACHER_LOSS = 5.0  # teacher imitation loss 权重；0 表示关闭。1.0 已实证会让 PPO 完全不进 stop 区(见 approved_models/2026_07_13_spr_student_bc1)。
...
STD_MAX = 0.1  # 动作标准差上限；0.3 时 P(stop)/步只有 0.002,stop 区(±10px)平均 11 步就被噪声踢出。
