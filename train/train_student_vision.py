"""Train vision student with the local PPO implementation.
...
parser = argparse.ArgumentParser(description="Train vision student with custom PPO.")
...
def load_encoder(device: torch.device) -> nn.Module:
    ...
    model_name = str(choice["model"])
    model_cls = VISION_MODELS[model_name]
    ...
    if not bool(cfg.FREEZE_VISION):
        raise NotImplementedError("current vision student trainer only supports FREEZE_VISION=True")
    return model
...
def save_env0_image(env: BallPPOEnv):
    if env.camera is None:
        return
    image = env.camera.data.output["rgb"][0]
    if image.shape[-1] > 3:
        image = image[..., :3]
    ...
    write_png(image.permute(2, 0, 1).contiguous(), str(PROJECT_ROOT / "vision_env0_preview.png"))
    ...
    torch.save(
        {
            ...
            "std": std,
            "vision_choice": vision_choice_name(),
            "vision_model": str(vision_choice()["model"]),
            "vision_state": str(vision_choice()["state"]),
            "vision_ckpt": str(vision_choice()["ckpt"]),
            "vision_feature_dim": int(vision_choice()["dim"]),
            "info": info,
        },
        path,
    )
...
def main():
    ...
    teacher_coef = float(cfg.TEACHER_LOSS) if mode == "student" else 0.0
    teacher_min = float(cfg.TEACHER_LOSS_MIN) if mode == "student" else 0.0
    teacher_down_success = float(cfg.TEACHER_DOWN_SUCCESS) if mode == "student" else 0.0
    teacher_up_out = float(cfg.TEACHER_UP_OUT) if mode == "student" else 0.0
    teacher_up_timeout = float(cfg.TEACHER_UP_TIMEOUT) if mode == "student" else 0.0
