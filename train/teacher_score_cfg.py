"""任务 v3 teacher 配置(稠密位置分)。与 v1 teacher 同超参,只换奖励与输出目录。"""
from pathlib import Path


OUT_DIR = Path("./models/rl/teacher_score")  # 训练现场输出目录，不上传 Git。
RANDOM_STOP_OUT_DIR = Path("./models/rl/teacher_score_random_stop")  # --random-stop 时的输出目录。
