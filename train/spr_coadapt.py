...
def agent_cfg_dict(repr_dim: int) -> dict:
    return {
        ...
        "spr_z_dim": int(cfg.SPR_Z_DIM),
        "spr_fpn": int(cfg.SPR_FPN),
        "spr_hidden": int(cfg.SPR_HIDDEN),
        "spr_pool": int(cfg.SPR_POOL),
        "spr_tau": float(cfg.SPR_TAU),
        "spr_k": int(cfg.SPR_K),
        "spr_coef": float(cfg.SPR_COEF),
        "enc_lr_ratio": float(cfg.ENC_LR_RATIO),
    }
...
def main() -> None:
    ...
    buffer = ReplayBuffer(
        ...
        store_images=(mode in ("pixels", "spr_coadapt")),  # spr_coadapt 每 update 重跑 SPR + 算 spr_loss 都要图。
    )
    ...
    try:
        for tick in range(1, total_ticks + 1):
            ...
            if tick % int(cfg.LOG_EVERY_TICKS) == 0:
                ...
                m = {k: (metric_sums.get(k, 0.0) / max(metric_count, 1)) for k in ("critic_loss", "actor_loss", "q1_mean", "q_target_mean", "spr_loss", "z_std")}
                row = {
                    "tick": tick,
                    "step": global_step,
