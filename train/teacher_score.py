...
import teacher_score_cfg as teacher_cfg
...
parser.add_argument("--action-k", type=float, default=None)
...
if args_cli.seed is not None:
    teacher_cfg.SEED = int(args_cli.seed)
...
from src.score_env import ScoreNoisePPOEnvCfg, ScorePPOEnvCfg, make_score_sb3_env
...
def make_out_dir() -> Path:
    out_dir = Path(str(teacher_cfg.RANDOM_STOP_OUT_DIR if args_cli.random_stop else teacher_cfg.OUT_DIR) + args_cli.tag)
...
def write_config(out_dir: Path):
    ...
    lines = [
        ...
        f"no_lateral = {bool(args_cli.no_lateral)}",
        f"obs_mask = {args_cli.obs_mask!r}",
        f"score_mode = {teacher_cfg_score_mode()!r}",
        f"score_action_k = {args_cli.action_k!r}",
...
def make_cfg() -> ScorePPOEnvCfg:
    end_d_min, end_d_max, end_x_min, end_x_max = end_range()
    cfg = ScorePPOEnvCfg()
    cfg.seed = int(teacher_cfg.SEED)
    cfg.episode_length_s = float(teacher_cfg.EPISODE_S)
    cfg.stop_n = int(teacher_cfg.STOP_N)
    cfg.scene.num_envs = int(args_cli.num_envs) if args_cli.num_envs else int(teacher_cfg.NUM_ENVS)
    if args_cli.action_k is not None:
        cfg.score_action_k = float(args_cli.action_k)
...
def main():
    ...
    env = make_score_sb3_env(make_cfg(), fast_variant=True)
    callbacks = []
    update_save = None
    ...
    try:
        model.learn(
            total_timesteps=int(args_cli.steps) if args_cli.steps else int(teacher_cfg.TOTAL_STEPS),
            callback=callbacks,
