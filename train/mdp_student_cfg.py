...
TEACHER_LOSS_MIN = 0.0  # teacher imitation loss 权重下限；0 表示允许完全关闭 teacher。
TEACHER_HOLD_STEPS = 250 * NUM_ENVS * N_STEPS  # teacher 保持满权重的采样步数（前 250 个 update）。
TEACHER_ANNEAL_STEPS = 150 * NUM_ENVS * N_STEPS  # 线性退到 0 的采样步数（第 251~400 个 update）。
