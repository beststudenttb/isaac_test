...
MDP_PATH = Path("./models/vision/mdp_state_teacher_50k/last.pt")  # 预训练 MDP state checkpoint。
FREEZE_MDP_BACKBONE = True  # 在线微调 MDP 时冻结 ResNet backbone。
TRAIN_MDP = True  # 是否在 PPO 中周期性微调 MDP。
MDP_WARMUP = 200  # 前多少 update 不训练 MDP，只训练策略。
MDP_EVERY = 50  # warmup 后每多少 update 训练一次 MDP。
MDP_UPDATES = 16  # 每次触发 MDP 训练时做多少次更新。
MDP_BATCH_ENVS = 4  # 每次 MDP 更新抽多少个 env 的轨迹。
